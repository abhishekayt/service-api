...
>(({ className, ...props }, ref) => (
    <thead
        ref={ref}
        className={cn("bg-slate-50/80 dark:bg-slate-800/50 [&_tr]:border-b", className)}
        {...props}
    />
));
...
>(({ className, ...props }, ref) => (
    <th
        ref={ref}
        className={cn(
            "h-10 px-4 text-left align-middle text-xs font-semibold text-slate-500 uppercase tracking-wider dark:text-slate-400 [&:has([role=checkbox])]:pr-0",
            className
        )}
        {...props}
    />
