...
export const ensureSettingsSeeded = async () => {
    const SETTING_SEED = [
        [1, "favicon", "Favicon (25*25)", "file", "application/favicon_194008.png"],
        [1, "logo", "Logo", "file", "application/logo_295352.png"],
        [1, "application_name", "Application Name", "text", "Adiyogi Fintech"],
    ...
    const ops = SETTING_SEED.map(([setting_type, setting_name, filed_label, filed_type, filed_value]) => ({
        updateOne: {
            filter: { setting_name },
            update: {
                $setOnInsert: { setting_type: Number(setting_type), setting_name, filed_label, filed_type, filed_value, status: 1 }
            },
            upsert: true
        }
    }));
